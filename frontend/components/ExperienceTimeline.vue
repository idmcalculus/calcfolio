<template>
    <section class="my-16 px-6 max-w-screen-xl mx-auto flex flex-col items-center" data-aos="fade-up">
		...
		<div
			...
			before:top-0 before:bottom-0 before:w-0.5 before:bg-primary before:dark:bg-primary
			before:-translate-x-1/2 before:z-0
		">
			<div
				v-for="(job, index) in experience"
				:key="index"
				class="mb-10 relative md:w-[calc(50%-theme(spacing.8))] /* Desktop: Half width minus gap */
					pl-[calc(theme(spacing.5)+theme(spacing.8))] md:pl-0 /* Mobile: Padding left */"
				:class="[!isEven(index) ? 'md:ml-[calc(50%+theme(spacing.8))]' : 'md:mr-auto']"
			>
				<div
					:class="['mb-2 md:mb-0 md:absolute md:top-1/2 md:-translate-y-1/2', 'md:z-10',
					!isEven(index) ? 'md:right-[calc(100%+theme(spacing.16))] md:text-right' : 'md:left-[calc(100%+theme(spacing.16))] md:text-left']"
				>
				...
				<div
					class="absolute left-0 top-[calc(50%+theme(spacing.3))] md:top-1/2 -translate-y-1/2 z-10 w-10 h-10 flex items-center justify-center
					p-1 rounded-full shadow-md border border-gray-200 dark:border-gray-700 bg-white dark:bg-darkBg"
					:class="[!isEven(index) ? 'md:left-[calc(-1.1*theme(spacing.12))]' : 'md:left-auto md:right-[calc(-1.1*theme(spacing.12))]']"
				>
				...
				<div
                    class="w-full md:max-w-lg relative bg-white dark:bg-zinc-900 shadow-md rounded-lg p-4 border border-gray-200 dark:border-gray-700
                        before:content-[''] before:absolute before:top-1/2 before:-translate-y-1/2 before:h-0.5 before:bg-primary before:dark:bg-primary before:z-0
                        before:left-[calc(-1*theme(spacing.4))] before:w-[theme(spacing.4)] /* Mobile default: connector starts from left edge */"
                    :class="[isEven(index) ? 'md:ml-auto md:before:left-auto md:before:right-[calc(-1*theme(spacing.4))]' : '']"
                >
